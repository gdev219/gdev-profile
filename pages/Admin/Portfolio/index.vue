...
<div class="Portfolio-admin">
  <h3 class="Portfolio-admin-header">
    <span class="btn-mode" v-bind:class="{'insert':modeFlag}" v-on:click="onChangeMode()">{{(modeFlag)?"수정 모드":"입력 모드"}}</span>
  </h3>
  <form v-on:submit.prevent="onSubmit(portfolioPreview)"  @keydown.enter.prevent.self>
    <label>프로젝트</label>
    <p  v-if="!modeFlag">
      <input type="text" name="title" v-model="portfolioPreview.title" id="title" placeholder="프로젝트 이름">
    </p>
    <p  v-else>
      <select v-model="portfolioPreview.title"  v-on:change="onChangeselect()">
        <option v-for="(title,index) in titleList" :value="title" v-bind:key="index">{{title}}</option>
      </select>
    </p>
    <label>종류</label>
    <p>
      <input type="text" name="text" v-model="portfolioPreview.type" id="type" placeholder="종류">   
    </p>
    <label>개발 기간</label>
    <p>
      <select v-model="portfolioPreview.date.start.year" id="date-start-year">
        <option value="" disabled selected>시작년</option>
        <option v-for="index in yearCount" :key="index">{{2010+index}}</option>
      </select>

      <select v-model="portfolioPreview.date.start.month" id="date-start-month">
    ...
    <label>주요 기능</label>
    <div>
      <input type="text" v-on:keydown.enter="onEnterPointlist(pointText)" v-model="pointText"/>
      <ul>
        <li v-for="point in portfolioPreview.pointlist" v-bind:key="point">{{ point }}</li>
      </ul>
    </div>

    <label>기술</label>
    <div>
      <input type="text" v-on:keydown.enter="onEnterTechlist(techText)" v-model="techText"/>
      <ul class="Portfolio-admin-techlist">
        <li v-for="(tech, index) in portfolioPreview.techlist" 
            v-bind:key="tech.text"
            v-bind:class="{'my-work':tech.duty}" 
            @click="addTeclistClass(index)">{{ tech.text }}</li>
      </ul>
    </div>
    <label>이미지 *The first image is a thumbnail</label>
    <p>
      <input type="file" name="이미지" @change="uploadImage($event.target.files)" accept="image/*">
    </p>
...
<script>
import { db, storage, serverTime } from "@/plugins/firebaseInit.js";
export default {
  components: {},
  data() {
    return {
  ...
  methods: {
    onSubmit(data) {
      ...
      this.portfolioPreview.timestamp = serverTime;
      var batch = db.batch();
      var snippetData = {};
      var postData = {};
      snippetData = {
      ...
      var snippetRef = db.collection("Snippets").doc(data.title);
      var postsRef = db.collection("Portfolio").doc(data.title);
    ...
    onChangeMode() {
      let titleList;
      let self = this;
      const postsRef = db.collection("Portfolio");
      //modeFlag insert: false update: true;
      self.modeFlag = !self.modeFlag;
      // clear object
      self.portfolioPreview = {
        title: "",
        type: "",
        date: { start: { year: "", month: "" }, end: { year: "", month: "" } },
        desc: "",
      ...
      if (self.modeFlag) {
        postsRef.get().then(function(snapshot) {
          snapshot.forEach(doc => {
            self.titleList.push(doc.data().title);
            self.dataList[doc.data().title]=doc.data();
          });
    ...
    onChangeselect() {
      let self = this;
      self.portfolioPreview = self.dataList[self.portfolioPreview.title];
    },
    ...
    onEnterTechlist(text) {
      this.portfolioPreview.techlist.push({ text: text, duty: false });
      this.techText = "";
    },
    addTeclistClass(index) {
      console.log(index);
      this.portfolioPreview.techlist[index].duty = !this.portfolioPreview
        .techlist[index].duty;
    },
    uploadImage(fileList) {
      ...
      //upload image
      if (fileList) {
        self.uploading = true; //prevent submit while its uploading
        ...
        ref
          ...
          .then(snapshot => {
            ...
            snapshot.ref.getDownloadURL().then(url => {
              // console.log(self.portfolioPreview);
              if (url) self.portfolioPreview.images.push(url);
            });
          });
      }
    }
