      ...
      <ul>
          <li v-for="(n, i) in (currentYear-birthYear+1)" :key="i">
            ...
            <div class="timeline-content">
              <p v-for="item in data[birthYear+i]" :key="item.month" v-bind:class="{'no-issue':!item.detail}">
                <span class="content-date">{{item.month+"월"}}<span v-if="item.day!=0">{{item.day+"일"}}</span></span>
